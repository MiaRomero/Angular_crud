...
describe('the requests at /bears', () => {
  before( (done) => {
    ...
    var newBear = new Bear({ name: 'testBear', variety: 'testing', location: 'testLand',
      continent: 'South_Test' });
    newBear.save( (err, data) => {
      if (err) return errorHandler(err);
      this.bear = data;
    });
    var newBear2 = new Bear({ name: 'Testy McTest', variety: 'testing', location: 'testTown',
      continent: 'North_Test' });
    newBear2.save( (err) => {
      if (err) return errorHandler(err);
      done();
    });
