...
describe('the routes at /tigers', () => {
  before( (done) => {
    ...
    var newTiger = new Tiger({ name: 'testTiger', variety: 'testing', location: 'jungle',
      continent: 'Asia' });
    newTiger.save( (err, data) => {
      if (err) return errorHandler(err);
      this.tiger = data;
    });
    var newTiger2 = new Tiger({ name: 'Tony', variety: 'cereal', location: 'Kellog',
  ...
  it('should update the tiger\'s information on a PUT request', (done) => {
    ...
    .send({ name: 'Hobbes', variety: 'cartoon', location: 'Calvin\'s house',
      continent: 'North_America' })
    .end( (err, res) => {
      expect(err).to.eql(null);
      expect(res.status).to.eql(200);
